...
class Env2DCylinderModified(gym.Env):
    ...
    def compute_reward(self,actionz=0):
        ...
        if self.reward_function == 'plain_drag':  # a bit dangerous, may be injecting some momentum
            ...
            #Ujet = abs(actionz[0])/rhoinf/Sa # Average jet velocity
            ...
            dPD = abs(PD0)-abs(PD)
            Pact = abs(actionz[0])**3/(Sa**2*rhoinf**2)
            self.PSR = dPD/Pact
            return dPD - Pact
            ...
            PD0 = mean_drag_no_control*Uinf # Power of drag from last action step
            dPD = abs(PD0)-abs(PD)
            actuations = np.mean(self.history_parameters["jet_0"].get()[-avg_length:])
            Pact = abs(actuations)**3/(Sa**2*rhoinf**2)
            self.PSR = dPD/Pact
            return dPD - Pact
